...
@Composable
fun MediaPlayerSelection(
    navigator: DestinationsNavigator,
    isOnboarding: Boolean = false
) {
    val viewModel = LocalHomeAssistantViewModel.current
    MediaPlayerSelector(
        entities = viewModel.mediaPlayers,
        onSelected = { mediaPlayer ->
            viewModel.updateSelectedMediaPlayer(mediaPlayer.entityId)
            if (isOnboarding)
                navigator.navigate(TriggerSelectionDestination(isOnboarding = true))
            else
